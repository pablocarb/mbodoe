function [p, err_per_round] = Parameter_Estimation_MBDoE_redGA(D, Y, model, n_p, Ap, bp, Aeqp, beqp, lbp, ubp, nonlconp)

    n_exp = size(D,1);
    n_rounds = size(Y,2);
    
    options = optimoptions('ga','Display','None','PopulationSize',20, 'MaxGenerations', 50);
